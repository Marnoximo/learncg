...
namespace Three_Layers_Model
{
    public partial class Form1 : Form
    {
        ...
        public Student_BLL bll;
        public Student_DAL sv_dal { get; set; }
        ...
        {
            InitializeComponent();
            bll = new Student_BLL(cntString);
            sv_dal = new Student_DAL(cntString);
            DataHelperInit();
            //SqlInit();
            CbbInit();
            ListViewInit();
        ...
        */
        ...
        private void btn_show_Click(object sender, EventArgs e)
        {
            //ShowRecords();
            //SetDataGridView();
            dgv_student.DataSource = bll.ListSV();
        }
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            try
            {
                if (WidgetsFilled() == false)
                {
                    throw new Exception("Please fill all needed information");
                }
                if (IsAvail(tb_id.Text))
                {
                        throw new Exception("Already has this id");
                }
                else
                {
                    Student student = new Student();
                    student.Id = tb_id.Text;
                    student.Name = tb_name.Text;
                    student.Class = cbb_class.SelectedItem.ToString();
                    student.Birthday = dtp_birthday.Value;
                    student.Gender = rb_male.Checked;
                    student.FacultyId = cbb_facultyid.SelectedItem.ToString();
